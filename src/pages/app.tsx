...
export default function AppPage({ onBack }: AppPageProps) {
  ...
  const [currentMedia, setCurrentMedia] = useState<HTMLImageElement | HTMLVideoElement | null>(null);
  const [mediaType, setMediaType] = useState<'image' | 'video' | null>(null);
  const [, setCropArea] = useState<{ x: number; y: number; width: number; height: number } | null>(null);
  const [error, setError] = useState<string | null>(null);

  const handleFrameCaptured = useCallback((media: HTMLImageElement | HTMLVideoElement, type: 'image' | 'video') => {
  ...
  const handleCropComplete = useCallback((newCropArea: { x: number; y: number; width: number; height: number }) => {
