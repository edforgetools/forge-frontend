 ...
 */
export async function exportUnder2MB(canvas, arg1, options) {
    const maxBytes = typeof arg1 === 'number' ? arg1 : 2 * 1024 * 1024;
    ...
    const attemptEncode = (c, q) => new Promise((resolve, reject) => {
        if (!c.toBlob)
            return reject(new Error('Canvas.toBlob not supported'));
        c.toBlob((blob) => (blob ? resolve(blob) : reject(new Error('Encode failed'))), 'image/jpeg', q);
    });
    const binarySearchQuality = async (c) => {
    ...
    if (work.width > minW || work.height > minH) {
        const atMin = downscale(work, Math.min(minW / work.width, minH / work.height));
...
export async function exportAndTrack(canvas, arg1) {
    try {
