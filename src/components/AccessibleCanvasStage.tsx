...
interface AccessibleCanvasStageProps {
  onDragStateChange?: (isDragging: boolean) => void;
}

export function AccessibleCanvasStage({
  onDragStateChange,
}: AccessibleCanvasStageProps) {
  ...
  // Get visible overlays for keyboard navigation
  const visibleOverlays = overlays.filter((overlay) => !overlay.hidden);

  // Enhanced keyboard navigation
  const handleCanvasKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLCanvasElement>) => {
      const step = event.shiftKey ? 10 : 1; // 10px with shift, 1px without
      let deltaX = 0;
      let deltaY = 0;

      // Handle overlay selection and navigation
        ...
        switch (event.key) {
          case "Tab":
            ...
            if (event.shiftKey) {
              ...
              setSelectedOverlayIndex((prev) =>
                prev <= 0 ? visibleOverlays.length - 1 : prev - 1
              );
            } else {
              // Next overlay
              setSelectedOverlayIndex((prev) =>
                prev >= visibleOverlays.length - 1 ? 0 : prev + 1
              );
            }
            const overlay = visibleOverlays[selectedOverlayIndex];
            if (overlay) {
              select(overlay.id);
              announce(`Selected ${overlay.type} overlay`);
            }
            return;
          ...
          case "Enter":
            ...
            if (
              ...
              selectedOverlayIndex < visibleOverlays.length
            ) {
              const overlay = visibleOverlays[selectedOverlayIndex];
              if (overlay) {
                select(overlay.id);
                announce(`Activated ${overlay.type} overlay`);
              }
            }
            return;
          ...
          case "Escape":
            ...
            announce("Deselected all overlays");
            return;
          ...
          case "Delete":
            ...
            if (selectedId) {
              ...
              if (overlay) {
                ...
                // remove(selectedId);
                announce(`Deleted ${overlay.type} overlay`);
              }
            }
            return;
        }
      ...
      // Handle overlay movement when an overlay is selected
      if (selectedId) {
        const overlay = overlays.find((o) => o.id === selectedId);
        if (overlay && !overlay.locked) {
          ...
          announce(
            `Moved overlay to position ${Math.round(overlay.x + deltaX)}, ${Math.round(overlay.y + deltaY)}`
          );
          return;
        }
      }
      ...
      switch (event.key.toLowerCase()) {
        ...
        case "g":
          event.preventDefault();
          // Toggle grid - this would need to be implemented
          announce(showGrid ? "Grid hidden" : "Grid shown");
          break;
        case "s":
          event.preventDefault();
          // Toggle safe zone - this would need to be implemented
          announce(showSafeZone ? "Safe zone hidden" : "Safe zone shown");
          break;
      }
    },
    [
  ...
  // Redraw function (same as original)
  ...
  // Draw overlay function (same as original)
  const drawOverlay = (
  ...
  // Draw grid function (same as original)
  const drawGrid = (
    ctx: CanvasRenderingContext2D,
    logicalWidth: number,
  ...
  // Draw safe zone function (same as original)
  const drawSafeZone = (
    ctx: CanvasRenderingContext2D,
    logicalWidth: number,
    logicalHeight: number
  ...
  // Draw crop overlay function (same as original)
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (clickedOverlay) {
      ...
      announce(`Selected ${clickedOverlay.type} overlay`);
    } else {
      select(undefined);
      setSelectedOverlayIndex(-1);
    }
  ...
  return (
    ...
    >
      <div className="relative bg-gray-100 rounded-2xl shadow-lg p-4 md:p-6 w-full max-w-4xl h-full">
        ...
        >
          ...
          <canvas
            ...
            tabIndex={0}
            role="img"
            aria-label={`Canvas for editing images and overlays. ${overlays.length} overlays present. ${selectedId ? "Overlay selected" : "No overlay selected"}. Use Tab to navigate overlays, arrow keys to move selected overlay, Delete to remove, Enter to activate.`}
            aria-describedby="canvas-description shortcuts-hint overlay-info"
          />
      ...
      {/* Canvas info - Enhanced for accessibility */}
      {hasContent && (
        <div
        ...
        >
          ...
          ratio • {overlays.length} overlays
        </div>
      )}

      {/* Overlay information for screen readers */}
      <div id="overlay-info" className="sr-only" aria-live="polite">
        {overlays.length === 0
          ? "No overlays present"
          : `${overlays.length} overlay${overlays.length > 1 ? "s" : ""} present. ${selectedId ? "Overlay selected" : "No overlay selected"}.`}
      </div>

      {/* Hidden hint for screen readers about shortcuts */}
      <div id="shortcuts-hint" className="sr-only" aria-hidden="true">
        Press the question mark key (?) to open keyboard shortcuts help. Use Tab
        to navigate overlays, arrow keys to move selected overlay, Delete to
        remove, Enter to activate.
      </div>
