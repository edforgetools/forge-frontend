...
export function useUndoRedo(maxHistory = 50) {
    ...
    const canRedo = currentIndex >= 0 && currentIndex < historyRef.current.length - 1;
...
export function useUndoRedoWithState(initialState, maxHistory = 50) {
    const [state, setState] = useState(initialState);
    const undoRedo = useUndoRedo(maxHistory);
